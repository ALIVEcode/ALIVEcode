...
const Activity = ({courseElement, editMode}: ActivityProps) => {
    ...
    const [confirmDelete, setConfirmDelete] = useState<"header" | "footer" | undefined>();
    ...
    const EditResource = ({resource}: { resource: Resource }) => {
        return (
            ...
            >
                <div
                    className="bg-[color:var(--bg-shade-two-color)] p-3 border-2 border-[color:var(--fg-shade-four-color)] rounded-sm">
    ...
    return (
        courseElement && (
            ...
            >
                ...
                <div
                    className="text-4xl bg-[color:var(--background-color)] mb-6 w-full border-[color:var(--bg-shade-four-color)]">
                ...
                />
                <AlertConfirm open={confirmDelete !== undefined}
                              setOpen={(value) => setConfirmDelete(value ? "header" : undefined)}
                              title={confirmDelete && t(`course.activity.remove_${confirmDelete}_confirm`)}
                              onConfirm={async () => {
