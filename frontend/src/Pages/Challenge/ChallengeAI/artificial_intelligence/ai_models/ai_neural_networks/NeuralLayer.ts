...
export class NeuralLayer {
	...
	public constructor(
		...
		for (let i: number = 0; i < nbNeurons; i++) {
			this.neurons.push(
				new Neuron(biases.getValue()[i][0], weights.getMatrixRow(i)),
			);
		}
	}
	...
	public computeLayer(inputs: Matrix): Matrix {
		...
		for (let i: number = 1; i < this.neurons.length; i++) {
			const newOutput: Matrix = this.neurons[i].computeOutput(
				inputs,
				this.activation,
			);
	...
	public setWeights(newWeights: Matrix) {
		if (this.weights.sameSize(newWeights)) this.weights = newWeights;
			...
			console.log(
				'Erreur Matrice : les tailles ne sont pas compatibles pour assignation.',
			);
	}
