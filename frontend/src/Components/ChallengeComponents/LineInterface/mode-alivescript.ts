...
import cacheLintInfoFR from './cache_lintinfo_fr.json';
import cacheLintInfoEn from './cache_lintinfo_en.json';
...
async function define(lang: SUPPORTED_LANG) {
	return await api.as.getLintInfo(lang);
}

let lintInfoFR: LinterFormatType = cacheLintInfoFR as LinterFormatType;
define(SUPPORTED_LANG.FR).then(resolve => {
	// const needUpdate = JSON.stringify(lintInfo) !== JSON.stringify(resolve);
	// needUpdate && console.log('Saving lintInfo: ' + JSON.stringify(resolve));
	lintInfoFR = resolve;
	/*needUpdate &&
		writeFile(
			'./frontend/src/Components/ChallengeComponents/LineInterface/cache_lintinfo_fr.json',
...
let lintInfoEN: LinterFormatType = cacheLintInfoEn as LinterFormatType;
define(SUPPORTED_LANG.EN).then(resolve => {
	// const needUpdate = JSON.stringify(lintInfo) !== JSON.stringify(resolve);
	// needUpdate && console.log('Saving lintInfo: ' + JSON.stringify(resolve));
	lintInfoEN = resolve;
	/*needUpdate &&
...
function generateHighlightRules(obj: any, lintInfo: LinterFormatType) {
...
ace.define(
	'ace/mode/alivescript_fr_highlight_rules',
	...
	(acequire: any, exports: any) => {
		...
		const AliveScriptFRHighlightRules = function (this: any) {
			generateHighlightRules(this, lintInfoFR);
		};

		oop.inherits(AliveScriptFRHighlightRules, TextHighlightRules);
		exports.AliveScriptFRHighlightRules = AliveScriptFRHighlightRules;
	},
);

ace.define(
	'ace/mode/alivescript_fr',
	[
		'require',
		'exports',
		'ace/lib/oop',
	...
	(acequire: any, exports: any) => {
		...
		const AliveScriptFRHighlightRules = acequire(
			'ace/mode/alivescript_fr_highlight_rules',
		).AliveScriptFRHighlightRules;

		var Mode = function (this: any) {
			this.HighlightRules = AliveScriptFRHighlightRules;
		};
...
(function () {
	ace.require(['ace/mode/alivescript_fr'], function (m: any) {
...
ace.define(
	'ace/mode/alivescript_en_highlight_rules',
	['require', 'exports', 'ace/lib/oop', 'ace/mode/text_highlight_rules'],
	(acequire: any, exports: any) => {
		const oop = acequire('ace/lib/oop');
		const TextHighlightRules = acequire(
		...
		const AliveScriptENHighlightRules = function (this: any) {
			generateHighlightRules(this, lintInfoEN);
		};

		oop.inherits(AliveScriptENHighlightRules, TextHighlightRules);
		exports.AliveScriptENHighlightRules = AliveScriptENHighlightRules;
	},
);

ace.define(
	'ace/mode/alivescript_en',
	[
		'require',
		'exports',
		'ace/lib/oop',
	...
	(acequire: any, exports: any) => {
		const oop = acequire('ace/lib/oop');
		const TextMode = acequire('ace/mode/text').Mode;
		const AliveScriptENHighlightRules = acequire(
			'ace/mode/alivescript_en_highlight_rules',
		).AliveScriptENHighlightRules;

		var Mode = function (this: any) {
			this.HighlightRules = AliveScriptENHighlightRules;
		};
...
(function () {
	ace.require(['ace/mode/alivescript_en'], function (m: any) {
		if (typeof module == 'object' && typeof exports == 'object' && module) {
			module.exports = m;
		}
	});
